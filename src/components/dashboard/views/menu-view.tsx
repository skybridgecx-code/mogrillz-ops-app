...
function Field({ label, error, help, wide, children }: { label: string; error?: string; help?: ReactNode; wide?: boolean; children: ReactNode }) {
  return <label className={`menu-editor__field${wide ? " menu-editor__field--wide" : ""}`}>
    <span>{label}</span>{children}{help ? <small>{help}</small> : null}{error ? <small className="menu-editor__error">{error}</small> : null}
  </label>;
}
