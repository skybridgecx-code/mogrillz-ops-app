  ...
  switch (result.error) {
    ...
    case "idempotency_conflict":
      return NextResponse.json(
        {
          error: "This idempotency key was already used for another order operation.",
    ...
    case "conflict":
      return NextResponse.json(
        {
          error: "This order already has that status. Refresh and try again.",
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    ...
    requestedStatus = normalizeOrderStatus(body?.status);
  } catch {
  ...
  if (error) {
    ...
    return NextResponse.json(
      {
        error: "Order status could not be updated. Verify the lifecycle migration is applied.",
        requestId,
      },
      { status: 500 },
    );
  ...
  if (!result.success) return transitionFailureResponse(result, requestId);
