...
- Server-only service-role access limited to bootstrap and storage administration
- Explicit mock-data mode with fictional customers and sample operations data

## Technology

**Next.js 16 · React 19 · TypeScript · Supabase SSR · PostgreSQL · Row Level Security · Node test runner · ESLint**
...
flowchart LR
    ...
    I --> J[Admin Bootstrap / Storage Administration]
```

Normal reads and writes use the authenticated Supabase client and remain subject to Row Level Security. The service-role client is imported from a `server-only` module and is requested only for privileged operations that cannot use the normal user session.
...
- Ordinary menu, inventory, and order-note mutations use the RLS-bound user client.
...
Before deploying the application, verify the production schema from a trusted environment:
...
Run the complete public quality gate:
